...
class DBAccessor:
    ...
    def get_number_of_records(self, database_name, table_name):
        """テーブルのレコード数を取得する
    ...
    def select_aggregate_information(self, database_name, question_id):
        """集計結果を取得する
        ...
        Raises:
            SqliteException: データベースアクセスに失敗した場合に発生する例外
            NoDataException: idに対応するデータが存在しない場合に発生する例外
        ...
        with sqlite3.connect(database_name) as connection:
            ...
            for i in range(len(aggregate_results)):
                aggregate_results_list.append(
                    AggregateResultsInformation(
                        Pai.get_values(aggregate_results[i][0]), aggregate_results[i][1]
                    )
                )

            return aggregate_results_list

    def select_question_information(self, database_name, question_id):
        """問題情報を取得する
    ...
    def select_tehai_information(self, database_name, tehai_id):
        """手牌情報を取得する
    ...
    def insert_answer_information(self, database_name, answer):
        """回答情報を登録する
        ...
        with sqlite3.connect(database_name) as connection:
            ...
            cursor.execute(
                ...
                [answer.question_id, player_id, answer.answer_pai_id.value.get("id")],
            )
